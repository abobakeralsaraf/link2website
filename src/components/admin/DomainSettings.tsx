      ...
      <DialogContent className="sm:max-w-[525px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {language === 'ar' ? 'إعدادات النطاق المخصص' : 'Custom Domain Settings'}
          </DialogTitle>
          <DialogDescription>
            {language === 'ar' 
              ? `ربط نطاق مخصص لموقع "${siteName}"`
              : `Connect a custom domain for "${siteName}"`}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex gap-2 sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={handleSaveDomain} disabled={isSaving}>
              ...
              {language === 'ar' ? 'حفظ النطاق' : 'Save Domain'}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
